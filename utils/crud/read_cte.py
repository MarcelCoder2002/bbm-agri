...
from utils.crud import (params)
from utils.crud.lib import get_pretty_name
...
class ColFilter:
    ...
    def get_dt_filters(self):
        ...
        for col in cols:
            ...
            start_c, end_c, btn_c = self.container.columns(
                [0.475, 0.475, 0.05], vertical_alignment="bottom"
            )

            default_start, default_end = params.get_dt_param(colname)

            start_key = f"{self.base_key}_date_filter_start_{label}"
            start = start_c.date_input(
                "Début",
                value=default_start,
                key=start_key,
                args=(colname, start_key, "start"),
                on_change=params.set_dt_param,
            )

            end_key = f"{self.base_key}_date_filter_end_{label}"
            end = end_c.date_input(
                "Fin",
                value=default_end,
                key=end_key,
                args=(colname, end_key, "end"),
                on_change=params.set_dt_param,
            )
            ...
            btn = btn_c.button(
                ...
                key=f"st_sql_{start_key}_{end_key}_cancel_btn",
            )
            if btn:
                st.query_params.pop(f"{colname}_start", None)
                st.query_params.pop(f"{colname}_end", None)
                st.rerun()
            ...
            result[colname] = start_date, end_date
