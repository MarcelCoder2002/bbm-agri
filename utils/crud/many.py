...
from utils.crud import lib, read_cte
from utils.crud.create_delete_model import CreateRow, DeleteRows
...
@st.fragment
def show_rel(conn: SQLConnection, Model, model_id: int | None, rel: RelationshipProperty):
    ...
    with st.expander(pretty_name):
        tab_read, tab_create, tab_delete = st.tabs(["Afficher", "Créer", "Supprimer"])
    ...
    with data_container:
        ...
        with tab_delete:
            ...
            if len(rows_id) == 0:
                st.text("Sélectionnez d’abord les lignes à supprimer dans l’autre onglet.")
            else:
...
def show_rels(conn: SQLConnection, Model, model_id: int | None = None):
